...
def writeInputfile( martensiteAmount, austeniteGrain, austeniteGrains, martensiteGrains, \
	...
	inputFile_name = 'Inputfile_'+ str(martensiteAmount)+'_'+ str(austeniteGrain[0])+'_'+ \
	str(laminate) +'.inp'		
	...
	with open(inputFile_name, 'a') as ifile:
		# ----- write sections -----
		for iGrain in austeniteGrains:
			# only write currently transforming grain once
			if austeniteGrain == iGrain: continue
		...
		for iGrain in martensiteGrains: 
		...
		string = '*Solid Section, elset=transig_' + str( austeniteGrain[0] ) + \
		  ', orientation=Ori_' +  str( austeniteGrain[0] ) + ', material=laminate' + \
		  str( laminate ) +'\n'
		ifile.write(string)
